...
class SingleDetectorConvNeXtGW(nn.Module):
    """
    Minimal-change single-detector / no-time version.

    Inputs:
      X: [B, 2, F, T] or [B, 1, F, T]
         - if 2 channels are provided, one detector is selected via detector_index
         - if 1 channel is provided, it is used directly

      t_feat: ignored, kept only for API compatibility

    Outputs:
      dict with same keys as before:
        logit_gw: [B]
        logits_3class: [B,3] or None
        attn_weights: None
    """
    def __init__(
        self,
        detector_index=0,       # 0 -> H1, 1 -> L1
        head_hidden=256,
        dropout=0.2,
        pretrained=True,
        use_aux_3class=False,
        ...
        # Same encoder family, now truly single-channel
        self.encoder = ConvNeXtTinySingleViewEncoder(
            pretrained=pretrained,
            in_chans=1,
        )
        ...
        self.fusion_norm = nn.LayerNorm(enc_dim)
        self.fusion_dropout = nn.Dropout(dropout)
    ...
    def forward(self, X, t_feat=None):
        # Accept either [B,2,F,T] or [B,1,F,T]
        if X.ndim != 4:
            raise ValueError(f"Expected X with shape [B,C,F,T], got {tuple(X.shape)}")

        if X.shape[1] == 2:
            if self.detector_index not in (0, 1):
                raise ValueError(f"detector_index must be 0 or 1, got {self.detector_index}")
            X = X[:, self.detector_index:self.detector_index + 1]   # [B,1,F,T]
        elif X.shape[1] == 1:
            pass
        else:
            raise ValueError(f"Expected 1 or 2 detector channels, got {X.shape[1]}")
        ...
        logit_gw = self.gw_head(z).squeeze(1)
        logits_3class = self.head_3class(z) if self.head_3class is not None else None
...
def main():
    ...
    model = SingleDetectorConvNeXtGW(
        detector_index=0,   # 0 = H1, 1 = L1
        pretrained=True,
        use_aux_3class=False,
        head_hidden=256,
    ...
    ckpt_path = "checkpoints11/best.pt"
    resume_path = None  # set to "checkpoints11/best.pt" to resume

    fig_dir = "training_figures11"
