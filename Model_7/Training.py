...
def multitask_gw_losses_and_metrics(
    outputs,
    y,
    lambda_aux=0.10,             # reduced from 0.30
    pos_weight_gw=None,          # set to ~2.0 for GW-vs-rest
    ...
    # Primary loss: plain weighted BCE
    loss_gw = F.binary_cross_entropy_with_logits(
        logit_gw,
        target_gw,
        pos_weight=pw,
        reduction="mean",
    ...
    # Auxiliary loss: keep it weak so it regularises without dominating
    ...
    with torch.no_grad():
        ...
        gw_f1 = 2 * gw_precision * gw_recall / (gw_precision + gw_recall + 1e-8)
...
def find_best_f1_threshold(y_true, p_gw, positive_class=2):
    """
    Choose threshold on validation set that maximises GW F1.
    """
    y_bin = (np.asarray(y_true) == positive_class).astype(np.uint8)
    p_gw = np.asarray(p_gw, dtype=np.float64)

    # Avoid pathological thresholds at exact endpoints
...
def print_optimiser_summary(optimiser):
    ...
    for group in optimiser.param_groups:
        ...
        print(f"  {name:<25} lr={lr:.2e}  params={group_params:,}")

    print(f"  {'TOTAL':<25} params={total_params:,}\n")
...
def print_convnext_structure(model):
    print("\n--- ConvNeXt encoder breakdown ---")

    features = model.encoder.features

    for i, block in enumerate(features):
        params = count_params(block)
        print(f"  features[{i}]  params={params:,}")

    print(f"  pre_logits     params={count_params(model.encoder.pre_logits):,}")
    print()
...
def main():
    ...
    LOSS_CFG = dict(
        lambda_aux=0.10,         # was 0.30
        pos_weight_gw=2.0,       # stronger push toward GW recall
        aux_label_smoothing=0.02,
    )
    ...
    {
        "name": "view_proj",
        "params": model.view_proj.parameters(),
        "lr": 2e-5,
    },
    {
        "name": "time_net",
        "params": model.time_net.parameters(),
        "lr": 2e-5,
    },
    {
        "name": "attn_gate",
        "params": model.attn_gate.parameters(),
        "lr": 2e-5,
    },
    {
        "name": "gw_head",
        "params": model.gw_head.parameters(),
        "lr": 8e-5,
    },
    {
        "name": "head_3class",
        "params": model.head_3class.parameters(),
        "lr": 8e-5,
    },
    ], weight_decay=2e-2)

    print_model_param_stats(model)
    print_optimiser_summary(optimiser)
    print_convnext_structure(model)
    ...
    scheduler = SequentialLR(
        optimiser,
        schedulers=[
            LinearLR(optimiser, start_factor=0.25, end_factor=1.0, total_iters=warmup_epochs),
            CosineAnnealingLR(optimiser, T_max=cosine_epochs, eta_min=1e-6),
        ],
    ...
    start_epoch = 0
    best_val = -float("inf")
    ckpt_path = "checkpoints7/best.pt"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        val_stats_raw = evaluate(
            ...
            device,
            return_outputs=True,
            gw_threshold=0.5,   # temporary, only for collecting outputs
            loss_cfg=LOSS_CFG
        )

        val_outs = val_stats_raw["outputs"]
        thr_info = find_best_f1_threshold(val_outs["y_true"], val_outs["p_gw"], positive_class=2)
        val_best_thr = thr_info["threshold"]

        # Recompute validation metrics using the selected threshold
        val_stats = evaluate(
            model,
            val_loader,
            device,
            return_outputs=False,
            gw_threshold=val_best_thr,
            loss_cfg=LOSS_CFG
        )
        ...
        history["val_best_thr"].append(float(val_best_thr))
        ...
        print(
            ...
            f"| val_acc3={val_stats['acc_3class']:.3f} "
            f"| GW P={val_stats['gw_precision']:.3f} "
            f"R={val_stats['gw_recall']:.3f} "
            f"F1={val_stats['gw_f1']:.3f} "
            f"| val_GW_AP={val_stats['gw_ap']:.4f} "
            f"| val_GW_AUC={val_stats['gw_roc_auc']:.4f} "
            f"| thr={val_best_thr:.3f}"
        )

        # save best on GW average precision (primary objective)
        val_metric = float(val_stats["gw_f1"])
        ...
        if np.isfinite(val_metric) and (val_metric > best_val):
            ...
            with open(os.path.join(fig_dir, "best_threshold.json"), "w") as f:
                json.dump({
                    "best_epoch": int(best_epoch),
                    "best_val_gw_f1": float(best_val),
        ...
        current_lrs = [pg["lr"] for pg in optimiser.param_groups]
        print(f"LRs: {[f'{lr:.2e}' for lr in current_lrs]} | val_best_thr={val_best_thr:.3f}")

        if epochs_without_improvement >= patience:
            print(f"[INFO] Early stopping triggered at epoch {epoch:03d}. Best epoch was {best_epoch:03d}.")
            break

    # final test eval (best checkpoint)
    ckpt = load_checkpoint(ckpt_path, model, optimiser=None, map_location=device)

    best_threshold_path = os.path.join(fig_dir, "best_threshold.json")
    if os.path.exists(best_threshold_path):
        with open(best_threshold_path, "r") as f:
            best_thr_info = json.load(f)
        test_threshold = float(best_thr_info["best_threshold"])
    else:
        test_threshold = 0.5

    print(f"[INFO] Using locked test threshold = {test_threshold:.4f}")
