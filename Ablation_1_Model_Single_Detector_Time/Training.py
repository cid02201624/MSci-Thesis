...
class SingleDetectorConvNeXtGWWithTime(nn.Module):
    """
    Minimal-change single-detector version.

    Inputs:
      X:      [B, 2, F, T]   (dataset still provides both detectors)
      t_feat: [B, 2]         (sin/cos of cyclic 24h GPS time)

    This model selects ONE detector internally:
      detector_idx = 0 -> H1
      detector_idx = 1 -> L1
    ...
    def __init__(
        ...
        if detector_idx not in (0, 1):
            raise ValueError(f"detector_idx must be 0 or 1, got {detector_idx}")

        self.detector_idx = detector_idx
        self.use_aux_3class = use_aux_3class
        self.resize_hw = resize_hw

        # Single-view encoder: now truly 1 input channel
        self.encoder = ConvNeXtTinySingleViewEncoder(
            pretrained=pretrained,
            in_chans=1,
        )
    ...
    def forward(self, X, t_feat):
        # Expect original dataset shape [B,2,F,T]
        if X.ndim != 4 or X.shape[1] != 2:
            raise ValueError(f"Expected X with shape [B,2,F,T], got {tuple(X.shape)}")

        # Select one detector only, keep channel dim -> [B,1,F,T]
        X = X[:, self.detector_idx:self.detector_idx + 1, :, :]
        ...
        return {
            "logit_gw": logit_gw,
            "logits_3class": logits_3class,
            "attn_weights": None,   # keep interface unchanged
        }
...
def main():
    ...
    single_detector_idx = int(os.environ.get("DETECTOR_IDX", "0"))

    model = SingleDetectorConvNeXtGWWithTime(
        detector_idx=single_detector_idx,
        pretrained=True,
        use_aux_3class=False,
        head_hidden=256,
        dropout=0.30,
    ...
    ckpt_path = "checkpoints13/best.pt"
    resume_path = None  # set to "checkpoints5/best.pt" to resume

    fig_dir = "training_figures13"
