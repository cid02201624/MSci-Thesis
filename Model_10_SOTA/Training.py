...
class CNN2DBlock(nn.Module):
    """
    Paper-inspired conv block:
      Conv -> ReLU -> BN
      Conv -> ReLU -> BN
      Conv -> ReLU -> BN
      MaxPool
    """
    def __init__(self, in_ch, out_ch, kernel_size=3, pool_kernel=2, pool_stride=2, dropout=0.0):
        super().__init__()
        pad = kernel_size // 2
...
class PaperStyleCNNEncoder2D(nn.Module):
    """
    2D adaptation of the paper's 1D CNN for spectrogram / time-frequency inputs.
    ...
    def __init__(
        ...
        block_dropout=0.05,
        head_dropout=0.30,
        input_hw=None,   # optional tuple (F, T) if you want exact dense sizing
    ):
        super().__init__()

        self.features = nn.Sequential(
            CNN2DBlock(in_chans,       base_filters[0], kernel_size=kernel_sizes[0], dropout=block_dropout),
            CNN2DBlock(base_filters[0], base_filters[1], kernel_size=kernel_sizes[1], dropout=block_dropout),
            CNN2DBlock(base_filters[1], base_filters[2], kernel_size=kernel_sizes[2], dropout=block_dropout),
            CNN2DBlock(base_filters[2], base_filters[3], kernel_size=kernel_sizes[3], dropout=block_dropout),
        )

        # Avoid hard-coding flatten size: use adaptive pooling
        self.pool = nn.AdaptiveAvgPool2d((4, 4))
        flat_dim = base_filters[-1] * 4 * 4

        self.pre_logits = nn.Sequential(
            nn.Flatten(),
            nn.Linear(flat_dim, dense_dims[0]),
            nn.ReLU(inplace=True),
            nn.BatchNorm1d(dense_dims[0]),
            nn.Dropout(head_dropout),

            nn.Linear(dense_dims[0], dense_dims[1]),
            nn.ReLU(inplace=True),
            nn.BatchNorm1d(dense_dims[1]),
            nn.Dropout(head_dropout),
        )
        self.out_dim = dense_dims[-1]
...
class JointPaperCNNGWWithTime(nn.Module):
    """
    Inputs:
    ...
    def __init__(
        ...
        self.encoder = PaperStyleCNNEncoder2D(
            in_chans=2,
            base_filters=(32, 64, 128, 256),
            kernel_sizes=(5, 3, 3, 3),
            dense_dims=(128, 64),
        ...
        enc_dim = self.encoder.out_dim  # 64
...
def multiclass_loss_and_metrics(outputs, y):
    logits = outputs["logits"]          # [B, C]
    loss = F.cross_entropy(logits, y)   # sparse categorical cross-entropy equivalent

    with torch.no_grad():
        yhat = logits.argmax(dim=1)
        acc = (yhat == y).float().mean()

    return loss, {"loss": loss.detach(), "acc": acc.detach()}
...
def main():
    ...
    model = JointPaperCNNGWWithTime(
        use_aux_3class=False,
        head_hidden=128,
        dropout=0.30,
        resize_hw=None,   # set e.g. (128, 128) only if you need it
    ).to(device)
    ...
    optimiser = AdamW(model.parameters(), lr=1e-3, weight_decay=1e-2)
    ...
    ckpt_path = "checkpoints10/best.pt"
    resume_path = None  # set to "checkpoints10/best.pt" to resume

    fig_dir = "training_figures10"
