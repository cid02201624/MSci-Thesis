...
Date: 26/02/2026
Description: Training loop for the two-head model, with time features. This is the main training loop 
...
MODIFIED: Mid-fusion cross-attention architecture for cross-detector coherence.
"""
...
# =====================================================================
# NEW: Split encoder — returns intermediate feature maps, not vectors
# REPLACES: ConvNeXtTinySingleViewEncoder
# =====================================================================

class ConvNeXtTinySplitEncoder(nn.Module):
    """
    ConvNeXt-Tiny split into early stages (0..split_after) and late stages.
    
    - forward_early(x)  -> feature map [B, C_mid, H', W']   (spatial structure preserved)
    - forward_late(x)   -> feature vector [B, out_dim]       (pooled)
    
    split_after=1 means stages [0,1] are early, stages [2,3] are late.
    For ConvNeXt-Tiny:
      stage 0 output: 96 channels
      stage 1 output: 192 channels   <-- split here by default
      stage 2 output: 384 channels
      stage 3 output: 768 channels
    """
    def __init__(self, pretrained=True, in_chans=1, split_after=1):
        super().__init__()
        ...
        # Replace stem conv for 1-channel input (same logic as before)
        ...
        # Split: features is a Sequential of 8 sub-modules in torchvision ConvNeXt:
        #   [0]=stem, [1]=stage0, [2]=downsample01, [3]=stage1,
        #   [4]=downsample12, [5]=stage2, [6]=downsample23, [7]=stage3
        # "split_after=1" means early = features[0..3], late = features[4..7]
        
        # Map split_after to the correct index in the features Sequential
        # stage 0 ends at index 1, stage 1 ends at index 3, stage 2 ends at index 5
        split_indices = {0: 2, 1: 4, 2: 6}
        split_idx = split_indices[split_after]

        all_features = list(base.features.children())
        self.early_stages = nn.Sequential(*all_features[:split_idx])
        self.late_stages = nn.Sequential(*all_features[split_idx:])

        self.avgpool = base.avgpool
        cls_layers = list(base.classifier.children())
        self.pre_logits = nn.Sequential(*cls_layers[:-1])
        self.out_dim = cls_layers[-1].in_features  # 768 for ConvNeXt-Tiny

        # Channel dim at the split point (needed for cross-attention)
        # stage 0->96, stage 1->192, stage 2->384
        self.mid_channels = {0: 96, 1: 192, 2: 384}[split_after]

    def forward_early(self, x):
        """x: [B, 1, H, W] -> feature map [B, C_mid, H', W']"""
        return self.early_stages(x)

    def forward_late(self, feat_map):
        """feat_map: [B, C_mid, H', W'] -> feature vector [B, out_dim]"""
        x = self.late_stages(feat_map)
        x = self.avgpool(x)
        x = self.pre_logits(x)
        return x
...
# =====================================================================
# NEW: Cross-detector attention at the feature-map level
# INSERT: new module
# =====================================================================

class CrossDetectorAttention(nn.Module):
    """
    Given spatial feature maps from H1 and L1 at an intermediate resolution,
    let each detector attend to the other's spatial features via cross-attention.
    This enables the model to learn cross-detector coherence (the defining
    feature of a real GW signal vs a single-detector glitch).
    """
    def __init__(self, dim, num_heads=4, dropout=0.1):
        super().__init__()
        self.cross_attn_h1 = nn.MultiheadAttention(dim, num_heads, dropout=dropout, batch_first=True)
        self.cross_attn_l1 = nn.MultiheadAttention(dim, num_heads, dropout=dropout, batch_first=True)
        self.norm_h1 = nn.LayerNorm(dim)
        self.norm_l1 = nn.LayerNorm(dim)

        # small FFN after cross-attn (standard transformer block pattern)
        ...
        self.norm_ffn_h1 = nn.LayerNorm(dim)
        self.norm_ffn_l1 = nn.LayerNorm(dim)

    def forward(self, feat_h1, feat_l1):
        """
        feat_h1, feat_l1: [B, C, H, W]  (intermediate feature maps)
        Returns: same shapes, enriched with cross-detector information
        """
        B, C, H, W = feat_h1.shape

        # flatten spatial dims -> [B, H*W, C]
        h1_flat = feat_h1.flatten(2).transpose(1, 2)
        l1_flat = feat_l1.flatten(2).transpose(1, 2)

        # cross-attention: each detector queries the other
        h1_cross, _ = self.cross_attn_h1(h1_flat, l1_flat, l1_flat)
        l1_cross, _ = self.cross_attn_l1(l1_flat, h1_flat, h1_flat)

        # residual + norm
        h1_out = self.norm_h1(h1_flat + h1_cross)
        l1_out = self.norm_l1(l1_flat + l1_cross)

        # FFN with residual
        h1_out = self.norm_ffn_h1(h1_out + self.ffn_h1(h1_out))
        l1_out = self.norm_ffn_l1(l1_out + self.ffn_l1(l1_out))

        # reshape back to feature maps
        h1_out = h1_out.transpose(1, 2).view(B, C, H, W)
        l1_out = l1_out.transpose(1, 2).view(B, C, H, W)

        return h1_out, l1_out


# =====================================================================
# NEW: Coherence branch — explicit cross-correlation feature
# INSERT: new module
# =====================================================================

class CoherenceBranch(nn.Module):
    """
    Computes a normalised cross-correlation map between H1 and L1 q-transforms
    and projects it to a small feature vector. This gives the model a direct,
    physics-informed signal: correlated detectors = likely real GW.
    """
    def __init__(self, n_freq_bins, out_dim=64, dropout=0.2):
        super().__init__()
        # The coherence map is [B, 1, F, T] — use a small 1D conv over frequency
    ...
    def forward(self, X):
        ...
        # normalise along time axis per frequency band
        h1_norm = (h1 - h1.mean(dim=-1, keepdim=True)) / (h1.std(dim=-1, keepdim=True) + 1e-6)
        l1_norm = (l1 - l1.mean(dim=-1, keepdim=True)) / (l1.std(dim=-1, keepdim=True) + 1e-6)

        # pointwise correlation map
        coherence_map = h1_norm * l1_norm  # [B, 1, F, T]

        return self.net(coherence_map)


# =====================================================================
# MODIFIED: Main model with mid-fusion
# REPLACES: MultiViewConvNeXtGWWithTime
# =====================================================================

class MultiViewConvNeXtGWWithTime(nn.Module):
    """
    Mid-fusion architecture for gravitational wave detection.
    
    Key difference from the original: instead of encoding each detector
    independently all the way to a vector and then fusing, we:
      1. Run early ConvNeXt stages on each detector independently
      2. Apply cross-detector attention on the spatial feature maps
      3. Run late ConvNeXt stages on the cross-attended features
      4. Fuse the final vectors with time embedding + coherence features
    
    This lets the model learn cross-detector coherence while spatial
    structure is still available — critical for separating real GW
    signals from single-detector glitches.
    """
    def __init__(
        self,
        time_dim=2,
        ...
        coherence_dim=64,
        split_after=1,           # split ConvNeXt after stage 1 (192-ch feature maps)
    ):
        super().__init__()
        self.use_aux_3class = use_aux_3class
        self.resize_hw = resize_hw

        # CHANGED: split encoder instead of full encoder
        self.encoder = ConvNeXtTinySplitEncoder(
            pretrained=pretrained, in_chans=1, split_after=split_after
        )
        enc_dim = self.encoder.out_dim       # 768
        mid_channels = self.encoder.mid_channels  # 192 at split_after=1

        # NEW: cross-detector attention at the feature-map level
        self.cross_attn = CrossDetectorAttention(
            dim=mid_channels,
            num_heads=cross_attn_heads,
            dropout=cross_attn_dropout,
        )

        # NEW: explicit coherence branch on raw input
        self.coherence_branch = CoherenceBranch(
            n_freq_bins=0,  # not used in current impl, kept for future
            out_dim=coherence_dim,
            dropout=dropout,
        )

        # Project encoder features into fusion space (same as before)
        ...
        # Gated attention over detector views (same as before)
        ...
        # CHANGED: fusion now includes coherence branch output
        # z_attn + z_mean + z_absdiff + time + coherence
        fusion_out_dim = fuse_dim + fuse_dim + fuse_dim + time_hidden + coherence_dim

        self.fusion_norm = nn.LayerNorm(fusion_out_dim)
        self.fusion_dropout = nn.Dropout(dropout)

        # Primary head: GW vs rest (same structure, wider input)
        ...
        # Auxiliary 3-class head (same structure)
    ...
    def forward(self, X, t_feat):
        """
        X:      [B, 2, F, T]
        t_feat: [B, 2]  (sin/cos cyclic GPS time)
        """
        B, V, H, W = X.shape
        if V != 2:
            raise ValueError(f"Expected exactly 2 detector views, got V={V}")

        # ---- Early stages: each detector independently ----
        ...
        feat_maps = self.encoder.forward_early(x)        # [B*2, C_mid, H', W']
        _, C_mid, Hf, Wf = feat_maps.shape
        feat_maps = feat_maps.view(B, V, C_mid, Hf, Wf)  # [B, 2, C_mid, H', W']

        feat_h1 = feat_maps[:, 0]  # [B, C_mid, H', W']
        feat_l1 = feat_maps[:, 1]

        # ---- NEW: cross-detector attention on spatial feature maps ----
        feat_h1, feat_l1 = self.cross_attn(feat_h1, feat_l1)

        # ---- Late stages: continue through remaining ConvNeXt stages ----
        feat_both = torch.stack([feat_h1, feat_l1], dim=1)      # [B, 2, C_mid, H', W']
        feat_both = feat_both.reshape(B * V, C_mid, Hf, Wf)
        z = self.encoder.forward_late(feat_both)                  # [B*2, enc_dim]

        z = self.view_proj(z)         # [B*2, fuse_dim]
        z = z.view(B, V, -1)         # [B, 2, fuse_dim]

        # ---- Time embedding (unchanged) ----
        t = self.time_net(t_feat)

        # ---- Attention fusion over views (unchanged) ----
        t_rep = t.unsqueeze(1).expand(-1, V, -1)
        gate_in = torch.cat([z, t_rep], dim=-1)
        attn_scores = self.attn_gate(gate_in).squeeze(-1)
        attn_weights = torch.softmax(attn_scores, dim=1)

        z_attn = (attn_weights.unsqueeze(-1) * z).sum(dim=1)
        z_mean = z.mean(dim=1)
        z_absdiff = (z[:, 0] - z[:, 1]).abs()

        # ---- NEW: coherence features from raw input ----
        coherence = self.coherence_branch(X)  # [B, coherence_dim]

        # ---- CHANGED: fused vector now includes coherence ----
        fused = torch.cat([z_attn, z_mean, z_absdiff, t, coherence], dim=1)
...
def evaluate(model, loader, device, return_outputs=False, gw_threshold=0.5, loss_cfg=None):
    ...
    # NEW: track sample indices in dataset order
    all_sample_idx = []
    ...
    for X, t_feat, y in loader:
        ...
        # NEW: dataset-order sample indices
...
def main():
    ...
    # CHANGED: model now uses mid-fusion architecture
    model = MultiViewConvNeXtGWWithTime(
        ...
        cross_attn_dropout=0.1,
        coherence_dim=64,
        split_after=1,           # cross-attend after ConvNeXt stage 1 (192-ch maps)
    ).to(device)

    # for p in model.encoder.parameters():
    #     p.requires_grad = False

    # CHANGED: added parameter groups for new modules
    optimiser = AdamW([
        {"params": model.encoder.parameters(), "lr": 1e-5},
        {"params": model.cross_attn.parameters(), "lr": 3e-5},       # NEW
        {"params": model.coherence_branch.parameters(), "lr": 3e-5}, # NEW
        {"params": model.view_proj.parameters(), "lr": 3e-5},
        {"params": model.time_net.parameters(), "lr": 3e-5},
        {"params": model.attn_gate.parameters(), "lr": 3e-5},
        {"params": model.gw_head.parameters(), "lr": 1e-4},
        {"params": model.head_3class.parameters(), "lr": 1e-4},
    ], weight_decay=1.5e-2)

    start_epoch = 0
    best_val = -float("inf")
    ckpt_path = "checkpoints6/best.pt"       # CHANGED: new checkpoint dir
    resume_path = None

    fig_dir = "training_figures6"             # CHANGED: new figures dir
    ...
    p_gw = outs["p_gw"]
    sample_index = outs["sample_index"] #new

    pred_df = pd.DataFrame({
        "sample_index": sample_index.astype(int),
        "true_class": y_true.astype(int),
        "pred_class": y_pred3.astype(int),
        "correct": (y_true == y_pred3),
    })

    pred_df.to_csv(os.path.join(fig_dir, "test_predictions.csv"), index=False)

    # Optional: save only mistakes too
    # pred_df.loc[~pred_df["correct"]].to_csv(
    #     os.path.join(fig_dir, "test_misclassifications.csv"),
    #     index=False,
    # )
