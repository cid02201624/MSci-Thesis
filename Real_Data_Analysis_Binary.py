...
pred = pd.read_csv("real_event_predictions/real_events_predictions9_340000.csv")
...
plot_df = plot_df.dropna(subset=["network_matched_filter_snr", "p_gw", "pred_gw_bin"])
...
# IMPORTANT, SNR against probability, color-coded by pred_gw_bin
plt.figure(figsize=(8, 6))

for cls, group in plot_df.groupby("pred_gw_bin"):
    plt.scatter(
...
noise_cluster = plot_df[
    (plot_df["pred_gw_bin"] == 0) &
    (plot_df["network_matched_filter_snr"] <= 15) &
    (plot_df["p_gw"] <= 0.55)
].copy()

low_snr_gw = plot_df[
    (plot_df["pred_gw_bin"] == 1) &
...
plt.scatter(
    plot_df["p_astro"],
    plot_df["p_gw"],
    c=plot_df["pred_gw_bin"].map(class_colours),
    alpha=0.7
)
...
plt.figure(figsize=(8, 6))
for cls, group in far_df.groupby("pred_gw_bin"):
    plt.scatter(
        group["far"],
        group["p_gw"],
        label=class_labels.get(int(cls), f"Class {int(cls)}"),
...
plt.figure(figsize=(8, 6))
for cls, group in gps_df.groupby("pred_gw_bin"):
    plt.scatter(
        group["gps_x"],
        group["p_gw"],
        label=class_labels.get(int(cls), f"Class {int(cls)}"),
...
plt.figure(figsize=(8, 6))
for cls, group in cm_df.groupby("pred_gw_bin"):
    plt.scatter(
...
plt.figure(figsize=(8, 6))
for cls, group in dist_df.groupby("pred_gw_bin"):
    plt.scatter(
...
# ------------------------------------------------------------------
# Histogram: GW vs Rest score distribution
# ------------------------------------------------------------------
...
plt.hist(
    ...
    label="Real GWOSC events"
)

plt.xlabel("Predicted Probability of GW")
plt.ylabel("Density")
plt.title("Score Distribution for Real Events")
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig("real_events_score_distribution.pdf", bbox_inches="tight", transparent=True)
plt.show()